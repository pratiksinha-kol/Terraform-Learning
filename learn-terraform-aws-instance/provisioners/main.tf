...
resource "aws_instance" "server" {
  ...
  /*   // Local-exec (saves private ip in a text file)
  provisioner "local-exec" {
    command = "echo ${self.private_ip} >> private_ips.txt"
  } */


  /*   // Remote-exec (saves private ip in a text file)
  ...
  // file provisioner  
  provisioner "file" {
    // source or content should be mentioned not BOTH, see below used content (ONLY ONE OF THEM)
    source      = "./copy.txt"
